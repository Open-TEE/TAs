		...
		while(count2--){
			...
			while(count--)
			{
				...
				if (payload.data) {
					...
					retVal = TEE_InvokeMGRCommand(TEE_TIMEOUT_INFINITE, COM_MGR_CMD_ID_TEST_COMM,
									  &payload, &returnPayload);

					if(retVal == TEE_SUCCESS && (returnPayload.size-sizeof(struct com_mgr_invoke_cmd_payload)) == payload.size)
					{
						bool check = true;
						unsigned int n;
					...
					} else if(retVal == TEE_SUCCESS && returnPayload.size == 0 && returnPayload.size == payload.size) {
						/* this is ok */
					} else {
						return TEE_ERROR_COMMUNICATION;
					}
