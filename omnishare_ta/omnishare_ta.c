...
static TEE_ObjectHandle oms_RSA_keypair_object = NULL;

/* Cached root directory AES key */
static TEE_ObjectHandle oms_AES_key_object = NULL;
 ...
 * Function is using OmniShare TA spesific key for executing RSA operation.
				     ...
				     TEE_ObjectHandle *aes_key_object)
{
	...
	if (aes_key) {
		...
		tee_rv = TEE_GetObjectBufferAttribute(new_aes_key_object, TEE_ATTR_SECRET_VALUE, aes_key, (size_t *)aes_key_size);
		if (tee_rv != TEE_SUCCESS) {
			       ...
			       TEE_ObjectHandle *file_key)
{
	...
	for (i = 0; i < key_chain->key_count; i++) {
		...
		TEE_InitRefAttribute(&tee_aes_attr, TEE_ATTR_SECRET_VALUE, next_aes_key, next_aes_key_size);
					 ...
					 TEE_Param *params)
{
	...
	tee_rv = warp_aes_operation(new_file_key, TEE_MODE_ENCRYPT,
					 ...
					 TEE_Param *params)
{
	...
	tee_rv = warp_aes_operation(dir_key, TEE_MODE_DECRYPT,
			    ...
			    TEE_Param *params)
{
	...
	tee_rv = get_file_key((struct key_chain_data *)&params[0], &file_key);
	if (tee_rv != TEE_SUCCESS)
		return tee_rv;
				  ...
				  TEE_Param *params)
{
	...
	return warp_oms_RSA_operation(TEE_MODE_ENCRYPT, aes_key, aes_key_size,
